...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //validación del email para el registro
    ...
    elseif (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{8,}$/", $password)) {
        ...
        //encriptación de la contraseña del usuario en la base de datos
        $password_hash = password_hash($password, PASSWORD_BCRYPT);

        try {
            $sql = "INSERT INTO empresas (cif, nombre_empresa, email, telefono, direccion, contraseña) 
                VALUES (:cif, :nombre, :email, :telefono, :direccion, :password)";
            ...
            $_SESSION["mensaje"] = "Empresa registrada correctamente. Ya puedes iniciar sesión.";
    ...
    <div class="flex-grow flex justify-center items-center px-4 mt-10 mb-10">
      <div class="bg-white p-6 sm:p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <form method="POST" action="">
          ...
          <label class="block font-medium mt-4">Nombre de la empresa:</label>
          ...
          <input type="text" name="direccion" class="w-full mt-1 px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-blue-300">

          <button type="submit" class="w-full bg-blue-600 text-white py-2 mt-6 rounded-md hover:bg-blue-700 transition">
            Registrarse
          </button>
        </form>

        <p class="text-center mt-6 text-gray-600">¿Ya tienes cuenta?<a href="login_empresa.php" class="text-blue-600 hover:underline">Inicia sesión aquí</a></p>
      </div>
    </div>
